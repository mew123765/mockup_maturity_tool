...
.detail-card {
    ...
    border: 1px solid #dee2e6; color: #111111 !important; margin-bottom: 10px;
}
...
.scroll-box { max-height: 150px; overflow-y: auto; padding-right: 5px; }

/* Yokoten & Folder Buttons */
.yokoten-box {
    width: 100%; padding: 20px; display: flex; align-items: center; justify-content: center;
    border-radius: 8px; font-weight: 800 !important; color: white !important;
    text-transform: uppercase; text-align: center; font-size: 1.1rem; margin-bottom: 10px;
}
...
.folder-btn {
    display: block; width: 100%; padding: 15px; background-color: #0078D4; 
    color: white !important; text-align: center; border-radius: 8px; 
    text-decoration: none; font-weight: bold; font-size: 0.9rem; border: 1px solid #005a9e;
}
</style>
...
if not df.empty:
    ...
    with col_h2:
        # Safer Filter logic for Green cases
        status_col = 'Status' if 'Status' in df.columns else None
        if status_col:
            green_mask = df[status_col].astype(str).str.strip().str.lower() == 'green'
